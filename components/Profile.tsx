...
export const Profile = () => {
  ...
  useEffect(()=>{
    async function getBasicDetails(){
      const result = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/get/basicdata`,{Email:storeEmail.Email})
      if(result.data){
        setProfile((prev)=>[...prev,result.data.allBasicDetails])
      }
    }
    async function getTestimonialDetails(){
      const result = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/get/testimonialdata`,{Email:storeEmail.Email})
      if(result.data){
  ...
  return (
    <div className="flex flex-row gap-2 px-2 overflow-x-scroll relative md:pr-4">
      {profile?.map((cur,index) => {
        return (
          <div key={index} className="border border-black bg-yellow-300 flex flex-col items-center py-[50px] min-w-full sm:min-w-[280px] rounded-lg">
            ...
            <p className="bg-black text-white px-2 rounded-lg mt-1">
              {cur.Role}
            </p>
            ...
            <p className="w-[80%] md:w-full text-center mt-4">
             {review}
            </p>
